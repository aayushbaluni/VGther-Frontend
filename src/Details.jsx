...
const HandleChange=async(e)=>{
    ...
    if(notes.length!==0){
        console.log(notes)
        if(times%5===0){
            checkoutHandler(notes)
        }
        else
        checkoutHandler(notes)
    }
}
const checkoutHandler=async(notes)=>{
  try{
...
<Box display="flex" alignItems="center" marginTop={2}>
            ...
            {haveReferralCode && (
              <>
                <Input isDisabled={code!=="NA"} marginLeft={2} id="refer" variant={'outline'} color={'white'} placeholder='Enter Referral Code' focusBorderColor='white' textColor={'white'} w={['60%', '30%']} type="tel"
              pattern="[0-9]{10}" />
...
//code=="NA"?times%5==0?amount-50*times:amount:times%5==0?amount-150*times:amount-100*times
